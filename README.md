...
Daily aggregator of California-located taildragger classified listings,
...
[Arkansas](https://github.com/taildraggers/arkansas) repos' approach, with
the state filter swapped to "CA".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, CA` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in California"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/california/`
...
```html
<iframe
  src="https://taildraggers.github.io/california/"
  title="Taildragger Ads in California"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_CALIFORNIA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
