"""Aggregates California-located listings from every taildraggers.com
...
ones whose location ends in ", CA".
...
This repo is a straight copy of the companion Alabama/Alaska/Arizona/Arkansas repos'
approach, with the state filter swapped to "CA".
"""
...
PAGE_TITLE = "Taildragger Ads in California"
STATE_ABBR = "CA"
...
_CALIFORNIA_LOCATION_RE = re.compile(r",\s*CA\s*$", re.IGNORECASE)


def _is_california(location: str) -> bool:
    return bool(_CALIFORNIA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        california_rows = [row for row in rows if _is_california(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(california_rows)} in California")
        all_listings.extend(california_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No California listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique California listings: {len(listings)}")
